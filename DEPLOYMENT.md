...
6. Hostinge viešinkite HTML puslapius, `robots.txt`, `sitemap.xml` bei `assets` ir `css` katalogus. Neviešinkite `.env`, `supabase`, `tests`, Python failų, `assets/supabase-config.example.js` ar projekto įrankių katalogų. Prijungus nuosavą domeną atnaujinkite domeną `robots.txt`, `sitemap.xml` ir `PUBLIC_SITE_URL`; `robots.txt` turi būti pasiekiamas domeno šaknyje.
...
10. Prijunkite mokėjimo ir transakcinio el. pašto teikėjus. Iki tol mokėjimo mygtukas lieka išjungtas, o elektroninių formų patvirtinimai nėra automatiškai siunčiami el. paštu.
