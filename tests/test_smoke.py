...
class AtminimasSmokeTests(unittest.TestCase):
    ...
    def test_homepage_has_qr_and_multi_service_flows(self):
        ...
        self.assertIn('href="parduotuve.html?product=metal">Užsakyti QR lentelę</a>', html)
        ...
        self.assertIn("Pasirinkite lentelę", html)
        self.assertIn("Sukurkite puslapį", html)
        self.assertIn("Apmokėkite per Paysera", html)
    ...
    def test_shop_offers_steel_colors_and_patterns(self):
        html = (ROOT / "parduotuve.html").read_text(encoding="utf-8")
        self.assertNotIn('value="asa"', html)
        self.assertNotIn("3D", html)
        self.assertIn("5 × 5 cm", html)
        self.assertEqual(html.count('name="plaque_color"'), 3)
        self.assertEqual(html.count('name="plaque_pattern"'), 4)
        self.assertIn('id="product-selection"', html)
        self.assertIn('id="product-image"', html)
        self.assertIn('assets/plaque-design.js', html)
    ...
    def test_selected_product_reaches_order_and_admin(self):
        ...
        self.assertIn('return "metal";', catalog)
        self.assertIn("selectedPlaqueDesign", editor)
        self.assertIn('createButton.href = "redaktorius.html?product=digital"', user)
        self.assertIn('preorderButton.href = "parduotuve.html?product="', user)
        self.assertNotIn("steel: {", editor)
        self.assertIn("data.product_type = productType", editor)
        self.assertIn('action: "create_order"', api)
        self.assertNotIn("steel_price", admin)
        self.assertNotIn("asa_available", admin)
        ...
        self.assertIn("Puslapio juodraštį galite kurti; prieš užsakymą reikės patikrinti kainą", editor)
        self.assertIn('preorderLink.href = "apmokejimas.html?order="', editor)
        self.assertIn('createLink.href = "redaktorius.html?product="', shop)
    ...
    def test_admin_has_separate_all_orders_dashboard(self):
        ...
        self.assertIn("await AtminimasAuth.isAdmin(me)", admin)
    ...
    def test_product_payments_use_paysera_and_historical_webhook_stays_verified(self):
    ...
    def test_editor_is_responsive_and_has_natural_color_palette(self):
        ...
        self.assertIn('data-editor-step-button="preview"', page)
        ...
        self.assertIn("target.scrollIntoView({ behavior: behavior", editor)
    ...
    def test_editor_has_three_mobile_steps_reorder_and_save_progress(self):
        page = (ROOT / "redaktorius.html").read_text(encoding="utf-8")
        editor = (ROOT / "assets" / "redaktorius.js").read_text(encoding="utf-8")
        api = (ROOT / "assets" / "atminimas-duomenys.js").read_text(encoding="utf-8")
        styles = (ROOT / "css" / "styles.css").read_text(encoding="utf-8")
        ...
        self.assertNotIn('data-editor-step="files"', page)
    ...
    def test_guest_builds_locally_and_signs_in_only_to_save_a_page(self):
        ...
        self.assertIn('store.openCursor()', editor)
        self.assertIn('String(cursor.key).indexOf(DRAFT_FILE_PREFIX) === 0', editor)
        self.assertIn("await discardCurrentDraft();", editor)
        self.assertGreaterEqual(editor.count("await discardSavedDraft();"), 2)
