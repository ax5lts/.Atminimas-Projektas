...
function email(value: unknown) {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(result)) {
    throw new HttpError("Įrašykite galiojantį el. paštą");
  }
  return result;
}
...
function cleanPriceKeys(value: unknown, allowed: readonly string[]) {
  ...
  const result = Array.from(
    new Set(
      value.map((item) => String(item || "")).filter((item) =>
...
function priceCatalog(settings: Settings) {
  const source =
    settings.price_catalog && typeof settings.price_catalog === "object"
      ? settings.price_catalog
      : {};
...
function calculateEstimate(
  ...
  const uniqueKeys = Array.from(
    new Set(keys.filter((key) => ALL_PRICE_KEYS.includes(key))),
  );
  ...
  if (hasDestination && hasBase) {
    ...
    oneWayMaxKm = Math.max(
      oneWayMinKm,
      roundUpFive(straightKm * Number(settings.road_factor_max)),
    );
    roundTripMinKm = oneWayMinKm * 2;
    roundTripMaxKm = oneWayMaxKm * 2;
    if (oneWayMaxKm > Number(settings.manual_review_over_one_way_km)) {
      reasons.push("distance_limit");
    }
    ...
    if (!Number.isInteger(rate) || Number(rate) < 0) {
      ...
      travelMinCents = Math.round(
        Math.max(0, roundTripMinKm - included) * Number(rate),
      );
      travelMaxCents = Math.round(
        Math.max(0, roundTripMaxKm - included) * Number(rate),
      );
  ...
  let calculated = reasons.length === 0 && serviceCents !== null &&
    travelMinCents !== null && travelMaxCents !== null;
  let totalMinCents: number | null = null;
  let totalMaxCents: number | null = null;
  if (
    calculated && serviceCents !== null && travelMinCents !== null &&
    travelMaxCents !== null
  ) {
    totalMinCents = serviceCents + travelMinCents;
    totalMaxCents = serviceCents + travelMaxCents;
  }
  if (
    (totalMinCents !== null && totalMinCents > MAX_CENTS) ||
    (totalMaxCents !== null && totalMaxCents > MAX_CENTS)
  ) {
    reasons.push("amount_limit");
    calculated = false;
    totalMinCents = null;
  ...
  const configurationMissing = reasons.some((reason) =>
    reason === "prices_missing" || reason === "travel_rate_missing" ||
    reason === "base_unconfigured"
  );
  return {
...
async function loadSettings(client = adminClient()) {
  const { data, error } = await client.from("service_quote_settings").select(
    "*",
  ).eq("id", "default").single();
  if (error || !data) {
    throw new Error(error?.message || "Paslaugų kainodara nerasta");
  }
  return data as Settings;
}

async function bodyOf(request: Request) {
  return await readJson(request, 64_000);
}

async function requireAdmin(request: Request) {
  const session = await requireUser(request);
  const client = adminClient();
  const { data, error } = await client.from("user_roles").select("role").eq(
    "user_id",
    session.user.id,
...
async function pseudonymousRateLimitHashes(request: Request) {
  ...
  const hash = async (scope: string, value: string) => {
    ...
    return Array.from(signature).map((byte) =>
...
async function consumeRequestRateLimit(
  ...
  if (/rate_limit_(?:ip|device)(?:\b|$)/i.test(error.message || "")) {
    throw new HttpError(
      "Per daug užklausų iš šio įrenginio arba tinklo. Pabandykite po valandos.",
      429,
    );
  }
  throw error;
...
function optionSelection(body: Record<string, unknown>, services: string[]) {
  ...
  const cleaning = cleanPriceKeys(
    body.cleaning_keys,
    PRICE_GROUPS.kapu_tvarkymas,
  );
  if (services.includes("zvakes") && candles.length !== 1) {
    throw new HttpError("Pasirinkite žvakių kiekį");
  }
  if (services.includes("geles") && flowers.length !== 1) {
...
async function createAction(request: Request, body: Record<string, unknown>) {
  ...
  const services = Array.isArray(body.services)
    ? Array.from(
      new Set(
        body.services.map((value) => String(value || "")).filter((value) =>
          ALLOWED_SERVICES.includes(value as typeof ALLOWED_SERVICES[number])
        ),
      ),
    )
    : [];
  ...
  const locationSource =
    hasCoordinates && ["registry", "saved"].includes(requestedSource)
      ? requestedSource
      : "manual";
  ...
  const { count: recentCount, error: rateError } = await client.from(
    "paslaugu_uzklausos",
  )
  ...
  if ((recentCount || 0) >= 5) {
    throw new HttpError(
      "Šiuo el. paštu šiandien jau pateikta per daug užklausų",
      429,
    );
  }
  const sessionEmail = String(session?.user.email || "").trim().toLowerCase();
  const ownerId =
    session?.user.email_confirmed_at && sessionEmail === contactEmail
      ? session.user.id
      : null;
  const payload = {
    owner_id: ownerId,
    ...
    paslaugos: services,
    mirusiojo_vardas: requiredText(
      body.deceased_name,
      2,
      180,
      "mirusiojo vardas",
    ),
    kapiniu_pavadinimas: requiredText(
      body.cemetery_name,
      2,
      200,
      "kapinių pavadinimas",
    ),
    savivaldybe: requiredText(
      body.municipality,
      2,
      160,
    ...
    geliu_pageidavimai: services.includes("geles")
      ...
      : null,
    zvakiu_pageidavimai: services.includes("zvakes")
      ? text(body.candle_details, 1200) || null
      : null,
    tvarkymo_pageidavimai: services.includes("kapu_tvarkymas")
      ? text(body.cleaning_details, 1600) || null
      : null,
  ...
  const { data, error } = await client.from("paslaugu_uzklausos").insert(
    payload,
  ).select(
    "id,quote_status,estimate_status,estimated_total_min_cents,estimated_total_max_cents,currency,created_at",
  ).single();
  if (error) throw error;
...
async function claimAction(request: Request, body: Record<string, unknown>) {
  ...
  if (existing.owner_id === user.id) {
    return { claimed: true, request: existing };
  }
  if (existing.owner_id) {
    throw new HttpError("Ši užklausa jau priskirta kitai paskyrai", 409);
  }
  if (
    String(existing.contact_email || "").toLowerCase() !==
      user.email.toLowerCase()
  ) {
    throw new HttpError(
      "Prisijunkite tuo pačiu el. paštu, kuriuo pateikėte užklausą",
      403,
    );
  }
  ...
  if (!claimed) {
    throw new HttpError(
      "Užklausos priskyrimo būsena pasikeitė. Atnaujinkite puslapį.",
      409,
    );
  }
...
async function saveSettingsAction(
  ...
  const client = adminClient();
  const sourceCatalog =
    body.price_catalog && typeof body.price_catalog === "object" &&
      !Array.isArray(body.price_catalog)
  ...
  const catalog = Object.fromEntries(ALL_PRICE_KEYS.map((key) => {
    ...
    if (!Number.isInteger(value) || value < 0 || value > 10000000) {
      throw new HttpError(`Netinkama kaina: ${key}`);
    }
    return [key, value];
  }));
  const baseLatitude = finite(body.base_latitude);
  const baseLongitude = finite(body.base_longitude);
  if (!validCoordinates(baseLatitude, baseLongitude)) {
    throw new HttpError("Netinkamos bazės koordinatės");
  }
  const factorMin = finite(body.road_factor_min);
  const factorMax = finite(body.road_factor_max);
  if (
    factorMin === null || factorMax === null || factorMin < 1 ||
    factorMax < factorMin || factorMax > 3
  ) throw new HttpError("Netinkami kelio koeficientai");
  const includedKm = finite(body.included_round_trip_km);
  const reviewKm = finite(body.manual_review_over_one_way_km);
  const rateValue = body.travel_rate_cents_per_km === null ||
      body.travel_rate_cents_per_km === ""
    ? null
    : Number(body.travel_rate_cents_per_km);
  if (
    includedKm === null || includedKm < 0 || reviewKm === null || reviewKm <= 0
  ) throw new HttpError("Netinkamos kelionės ribos");
  if (
    rateValue !== null &&
    (!Number.isInteger(rateValue) || rateValue < 0 || rateValue > 100000)
  ) throw new HttpError("Netinkamas kilometro tarifas");
  const update = {
  ...
  const { data, error } = await client.from("service_quote_settings").update(
    update,
...
async function sendQuoteAction(
  ...
  if (
    !requestId || !Number.isInteger(revision) ||
  ...
  if (error || !quote) {
    throw new Error(error?.message || "Pasiūlymo išsaugoti nepavyko");
  }
  ...
  try {
    ...
    if (!recipient && quote.owner_id) {
      const { data: account, error: accountError } = await client.auth.admin
        .getUserById(quote.owner_id);
    ...
    await sendEmail({
      ...
      paragraphs: [
        `Galutinė pasiūlymo kaina: ${
          money(quote.quote_amount_cents, quote.currency)
        }.`,
        quote.quote_message ||
        "Peržiūrėkite pasiūlymą kliento zonoje ir pasirinkite, ar jį priimate.",
        `Pasiūlymas galioja iki ${
          new Intl.DateTimeFormat("lt-LT", { dateStyle: "long" }).format(
            new Date(quote.quote_expires_at),
          )
        }.`,
      ...
      actionUrl: `${publicSiteUrl()}vartotojas.html?service=${
    ...
    emailError = emailFailure instanceof Error
      ? emailFailure.message
      : "Laiško išsiųsti nepavyko";
    const { error: errorUpdateError } = await client.from("paslaugu_uzklausos")
      .update({ quote_email_error: emailError.slice(0, 2000) }).eq(
        "id",
        quote.id,
      );
    if (errorUpdateError) {
      emailError =
        `${emailError}; būsenos išsaugoti nepavyko: ${errorUpdateError.message}`
          .slice(0, 2000);
    }
  }
...
function assertStripeSessionBinding(
  ...
  const metadata = session.metadata && typeof session.metadata === "object"
    ? session.metadata
    : {};
  const sameSession = service.payment_session_id === null ||
    session.id === service.payment_session_id;
  const valid = sameSession &&
    ...
    String(metadata.payment_attempt_id || "") ===
      String(service.payment_attempt_id) &&
    Number(session.amount_total) === service.quote_amount_cents &&
    String(session.currency || "").toUpperCase() ===
      String(service.currency || "EUR").toUpperCase() &&
    Number(session.expires_at) === expiresAtSeconds;
  if (!valid) {
    throw new HttpError(
      "Stripe mokėjimo sesijos duomenys neatitinka pasiūlymo",
      409,
    );
  }
}
...
async function readStripeSession(stripeKey: string, sessionId: string) {
  ...
  if (!response.ok) {
    throw new HttpError(
      "Esamos mokėjimo sesijos patikrinti nepavyko. Pabandykite dar kartą.",
      502,
    );
  }
...
async function startPaymentAction(
  ...
  if (user.is_anonymous) {
    throw new HttpError("Prieš mokėjimą prisijunkite", 401);
  }
  const requestId = String(body.request_id || "");
  if (!requestId) throw new HttpError("Trūksta paslaugos užklausos numerio");
  const stripeKey = env("STRIPE_SECRET_KEY", false);
  if (!stripeKey) {
    throw new HttpError("Mokėjimų tiekėjas dar nesukonfigūruotas", 503);
  }
  const { data: service, error } = await client.rpc(
    "begin_my_service_payment",
    {
      p_request_id: requestId,
      p_actor_id: user.id,
    },
  ...
  if (error || !service) {
    throw new Error(error?.message || "Mokėjimo bandymo pradėti nepavyko");
  }
  if (service.payment_status === "paid") {
    throw new HttpError("Pasiūlymas jau apmokėtas", 409);
  }
  if (service.quote_status === "expired") {
    throw new HttpError(
      "Pasiūlymo galiojimas baigėsi. Paprašykite naujo pasiūlymo.",
      409,
    );
  }
  if (
    service.quote_status !== "accepted" ||
    service.payment_status !== "processing" || !service.payment_attempt_id
  ) {
    throw new HttpError("Pirmiausia priimkite galutinį pasiūlymą", 409);
  }
  if (
    !Number.isInteger(service.quote_amount_cents) ||
    service.quote_amount_cents <= 0
  ) throw new HttpError("Galutinė kaina nepatvirtinta", 409);
  const sessionExpirySeconds = Math.floor(
    new Date(service.payment_session_expires_at).valueOf() / 1000,
  );
  if (!Number.isFinite(sessionExpirySeconds)) {
    throw new Error("Mokėjimo sesijos galiojimo laikas nenustatytas");
  }

  if (service.payment_session_id) {
    const existingSession = await readStripeSession(
      stripeKey,
      String(service.payment_session_id),
    );
    ...
    throw new HttpError(
      "Mokėjimo sesija nebegalioja. Atnaujinkite puslapį ir pabandykite dar kartą.",
      409,
    );
  }
  ...
  params.set(
    "success_url",
    `${site}vartotojas.html?service=${
      encodeURIComponent(service.id)
    }&payment=success#paslaugos`,
  );
  params.set(
    "cancel_url",
    `${site}vartotojas.html?service=${
      encodeURIComponent(service.id)
    }&payment=cancelled#paslaugos`,
  );
  ...
  params.set(
    ...
    String(service.currency || "EUR").toLowerCase(),
  );
  params.set(
    "line_items[0][price_data][unit_amount]",
    String(service.quote_amount_cents),
  );
  params.set(
    "line_items[0][price_data][product_data][name]",
    "Kapavietės priežiūros paslaugos",
  );
  params.set("line_items[0][quantity]", "1");
  params.set("locale", "lt");
  if (service.contact_email) {
    params.set("customer_email", String(service.contact_email));
  }
  ...
  const stripeResponse = await fetch(
    "https://api.stripe.com/v1/checkout/sessions",
    {
      ...
      headers: {
        ...
        "Idempotency-Key":
          `service-checkout-${service.id}-${service.quote_revision}-${service.payment_attempt_id}`
            .slice(0, 255),
      },
      body: params,
    },
  );
  const session = await stripeResponse.json().catch(() => ({})) as Record<
    string,
    any
  >;
  if (!stripeResponse.ok) {
    if (finalStripeClientError(stripeResponse.status, session)) {
      const { error: failError } = await client.rpc(
        "fail_unattached_service_payment",
        {
      ...
      if (failError) {
        throw new Error(
          `Stripe klaida; bandymo būsenos atkurti nepavyko: ${failError.message}`,
        );
      }
    }
    throw new HttpError(
      "Stripe mokėjimo sesijos sukurti nepavyko. Pabandykite dar kartą.",
      502,
    );
  }
  assertStripeSessionBinding(session, service, sessionExpirySeconds);
  const safeCheckoutUrl = checkoutUrl(session.url);
  if (!safeCheckoutUrl || !session.id) {
    throw new Error("Stripe negrąžino saugios mokėjimo nuorodos");
  }
  const { data: attached, error: attachError } = await client.rpc(
  ...
  if (attachError || !attached || attached.payment_session_id !== session.id) {
    throw new Error(
      attachError?.message || "Mokėjimo sesijos patvirtinti nepavyko",
    );
  }
  return {
