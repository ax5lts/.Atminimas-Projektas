...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (!orderId || !["invoice", "qr", "label"].includes(type)) {
      return json({ error: "Neteisinga užklausa" }, 400);
    }

    const { data: order } = await client.from("uzsakymai").select(
      "id,profilis_id,label_storage_path,profiliai!inner(owner_id)",
    ...
    if (type === "invoice") {
      const { data } = await client.from("invoice_documents").select(
        "storage_path",
      ).eq("order_id", order.id).maybeSingle();
      path = data?.storage_path || null;
    } else if (type === "qr") {
      const { data } = await client.from("production_jobs").select(
        "qr_pdf_path,qr_svg_path",
      ).eq("order_id", order.id).maybeSingle();
    ...
    const { data, error } = await client.storage.from("automation-documents")
      .createSignedUrl(path, 120);
    if (error || !data?.signedUrl) {
      throw error || new Error("Nepavyko sukurti nuorodos");
    }
    ...
    return json({ error: "Nepavyko atidaryti dokumento" }, 500);
  }
});
