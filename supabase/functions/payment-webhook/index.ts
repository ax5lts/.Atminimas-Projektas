...
async function verifyStripeSignature(
  ...
  const signatures = values.filter(([key]) => key === "v1").map(([, value]) =>
    value
  );
  ...
  const expected = Array.from(digest).map((byte) =>
...
Deno.serve(async (request: Request) => {
  ...
  if (!secret || !(await verifyStripeSignature(raw, signature, secret))) {
  ...
  try {
    ...
    const orderId = String(
      object?.metadata?.order_id || object?.client_reference_id || "",
    );
    ...
    if (entityType === "service") {
      if (
        !UUID_PATTERN.test(serviceRequestId) ||
        !Number.isInteger(quoteRevision) || quoteRevision < 0 ||
        !UUID_PATTERN.test(paymentAttemptId)
      ) {
        return json({ received: true, entity: "service", ignored: true });
      }
      const { data: serviceStatus, error: serviceError } = await client.rpc(
        "process_stripe_service_payment_event",
        {
    ...
    const { data: status, error: processError } = await client.rpc(
