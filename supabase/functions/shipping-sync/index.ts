...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    const { data: orders, error } = await client.from("uzsakymai")
      .select(
        "id,delivery_method,carrier,city,parcel_terminal,recipient_name,recipient_phone,recipient_email,product_type,shipment_provider_ref,tracking_number,shipping_status",
      )
    ...
    for (const order of orders || []) {
      try {
        results.push({
          order_id: order.id,
          ...(await syncShipment(client, order)),
        });
      } catch (syncError) {
        results.push({
          order_id: order.id,
