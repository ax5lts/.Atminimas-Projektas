...
const FONT_URL =
  "https://raw.githubusercontent.com/googlefonts/noto-fonts/ffebf8c1ee449e544955a7e813c54f9b73848eac/hinted/ttf/NotoSans/NotoSans-Regular.ttf";
...
export async function createInvoicePdf(input: {
  ...
  const left = 54;
  let y = height - 58;
  const text = (value: string, x: number, yy: number, size = 10, color = ink) =>
    page.drawText(value || "-", { x, y: yy, size, font: regular, color });
  const title = input.documentType === "vat_invoice"
  ...
  const sellerLines = [
    ...
    input.seller.activity_form,
    input.seller.registration_code
      ? `Kodas: ${input.seller.registration_code}`
      : "",
    input.seller.vat_code ? `PVM kodas: ${input.seller.vat_code}` : "",
    input.seller.address,
    input.seller.email,
  ].filter(Boolean) as string[];
  const buyerLines = [input.buyer.name, input.buyer.email, input.buyer.address]
    .filter(Boolean) as string[];
  sellerLines.forEach((lineText, index) =>
    text(lineText, left, y - 20 - index * 16, 9.5)
  );
  buyerLines.forEach((lineText, index) =>
    text(lineText, 320, y - 20 - index * 16, 9.5)
  );
  y -= 130;
  ...
  text(
    "Dokumentas sugeneruotas automatiškai po patvirtinto mokėjimo.",
    left,
    58,
    8.5,
    muted,
