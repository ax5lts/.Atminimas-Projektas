begin;
-- Nullable fields retain the actual (unknown) design of historical orders.
alter table public.uzsakymai
  add column product_color text check (product_color in ('gold','silver','black')),
  add column product_pattern text check (product_pattern in ('tree','heart','wings','star'));
alter table public.uzsakymai add constraint order_design_pair check (
  (product_color is null and product_pattern is null) or
  (product_color is not null and product_pattern is not null and product_type = 'metal')
);

-- Do not reprice historical orders when withdrawing a product.
alter table public.product_catalog disable trigger automation_refresh_product_orders;
update public.product_catalog set enabled = false where id = 'asa';
alter table public.product_catalog enable trigger automation_refresh_product_orders;

create function public.create_designed_product_order(
...
begin
  if p_actor_id is null or p_product_type is distinct from 'metal'
    ...
    or p_product_pattern is null or p_product_pattern not in ('tree','heart','wings','star')
...
-- Once selected, a different design is a separate order, including during payment.
create function private.lock_order_design() returns trigger
language plpgsql security invoker set search_path=public as $$
begin
  if row(old.product_color,old.product_pattern) is distinct from row(new.product_color,new.product_pattern)
    then raise exception 'Order design is locked'; end if;
  return new;
end; $$;
revoke all on function private.lock_order_design() from public,anon,authenticated;
create trigger lock_order_design before update on public.uzsakymai
  for each row execute function private.lock_order_design();
commit;
